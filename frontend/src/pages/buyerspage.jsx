...
function PartyModal({ isOpen, onClose, onSubmit, initialData, isEdit, type }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({
        submit: err.response?.data?.message || 'Failed to save buyer'
      });
    } finally {
      setLoading(false);
    }
...
function BuyersTable({ buyers, onEdit, onDelete, searchTerm }) {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-16 text-center">
          ...
          <p className="text-sm text-gray-400">
            {searchTerm ? 'Try a different search term' : 'Add your first buyer to get started'}
          </p>
        </div>
      </div>
    );
      ...
      {totalPages > 1 && (
        <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex items-center justify-between">
          <span className="text-sm text-gray-500">
            Showing {indexOfFirstItem + 1}–{Math.min(indexOfLastItem, filteredBuyers.length)} of {filteredBuyers.length} buyers
          </span>
...
export default function BuyersPage() {
  ...
  const fetchParties = async () => {
    ...
    try {
      ...
      showNotification('Failed to load buyers', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (formData) => {
    try {
      ...
      await fetchParties();
      showNotification('Buyer created successfully!', 'success');
    } catch (error) {
      throw error;
    }
  };
  ...
  const handleUpdate = async (formData) => {
    try {
      await partyApi.updateParty(selectedBuyer.id, formData);
      await fetchParties();
  ...
  const handleDelete = async () => {
    try {
      await partyApi.deleteParty(selectedBuyer.id);
      await fetchParties();
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-gray-500 font-medium">Loading buyers...</span>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-500">Manage your buyer relationships</p>
              </div>
            </div>

            <button
              onClick={() => setIsAddModalOpen(true)}
              className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-purple-700 transition-all flex items-center gap-2 shadow-lg shadow-blue-500/25"
            >
